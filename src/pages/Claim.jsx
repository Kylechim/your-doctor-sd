...
const SAMPLE_RESULTS = [
  { name: "Dr. Maria Elena Vasquez, MD", specialty: "Family Medicine", address: "835 3rd Ave, Chula Vista, CA 91910", npi: "1234567890", claimed: false },
  { name: "Dr. Maria Vasquez-Reyes, DO", specialty: "Internal Medicine", address: "1200 Third Ave, San Diego, CA 92101", npi: "1234567899", claimed: true },
];
        ...
        <div key={i} style={{ display: "flex", alignItems: "center" }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 5 }}>
            <div style={{
              ...
              background: i < current ? C.green : i === current ? C.ocean : "white",
              color: i <= current ? "white" : C.muted,
              border: `2px solid ${i < current ? C.green : i === current ? C.ocean : C.border}`,
              transition: "all 0.3s",
              zIndex: 1,
            }}>
              {i < current ? "✓" : i + 1}
          ...
          {i < STEPS.length - 1 && (
            <div style={{ width: isMobile ? 30 : 60, height: 2, background: i < current ? C.green : C.border, margin: isMobile ? "0 4px" : "0 4px", marginBottom: isMobile ? 0 : 20, transition: "background 0.3s" }} />
          )}
        </div>
      ))}
    </div>
...
function ToggleChip({ label, active, onClick }) {
  return (
    <button onClick={onClick} style={{
      ...
      background: active ? C.ocean : "white",
      color: active ? "white" : C.text,
      border: `1.5px solid ${active ? C.ocean : C.border}`,
      transition: "all 0.15s",
    }}>{label}</button>
  );
}

// ── STEP 1: Find Practice ──
function Step1({ data, setData, onNext }) {
  ...
  function doSearch() {
    if (search.trim().length > 1) setSearched(true);
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: "1.2rem" }}>
        <Input value={search} onChange={e => setSearch(e.target.value)} placeholder="Your name or NPI number…"
          onKeyDown={e => e.key === "Enter" && doSearch()}
          style={{ flex: 1 }}
        />
        <button onClick={doSearch} style={{ background: C.ocean, color: "white", border: "none", padding: "0 1.2rem", borderRadius: 9, fontFamily: "inherit", fontSize: 14, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" }}>
          Search
        </button>
      </div>
      ...
      {searched && (
        <div>
          <div style={{ fontSize: 12, color: C.muted, marginBottom: 10 }}>Found {SAMPLE_RESULTS.length} results for "{search}"</div>
          {SAMPLE_RESULTS.map((r, i) => (
            <div key={i} onClick={() => !r.claimed && setSelected(r)} style={{
              ...
              background: selected?.npi === r.npi ? "rgba(26,107,138,0.04)" : r.claimed ? "#fafafa" : "white",
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: C.muted }}>📍 {r.address}</div>
                  <div style={{ fontSize: 11, color: "#aac4ce", marginTop: 2 }}>NPI {r.npi}</div>
                </div>
                <div>
                  {r.claimed
                    ? <span style={{ fontSize: 11, padding: "3px 9px", borderRadius: 20, background: "#f5f5f5", color: C.muted, border: `1px solid ${C.border}` }}>Already claimed</span>
                    : selected?.npi === r.npi
          ...
          <div style={{ marginTop: "1rem", padding: "0.9rem", background: "rgba(26,107,138,0.04)", borderRadius: 10, border: `1px solid ${C.border}`, fontSize: 13, color: C.muted }}>
            🤔 Don't see your practice? <span style={{ color: C.ocean, fontWeight: 600, cursor: "pointer" }}>Request to be added →</span>
          </div>
        </div>
      )}
  ...
  return (
    <div>
      ...
      <div style={{ background: "rgba(26,107,138,0.05)", border: `1px solid rgba(26,107,138,0.15)`, borderRadius: 10, padding: "0.9rem 1rem", marginBottom: "1.4rem", fontSize: 13 }}>
        ...
        <div style={{ color: C.muted, marginTop: 2 }}>{data.practice?.address}</div>
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "1.1rem" }}>
        <div>
          ...
          {npiConfirm === data.practice?.npi && (
            <div style={{ fontSize: 12, color: C.green, marginTop: 4 }}>✅ NPI verified</div>
          )}
        </div>
        ...
        <div>
          ...
          <div style={{ fontSize: 11, color: C.muted, marginTop: 4 }}>We'll send a verification link to this address.</div>
        </div>

        <div>
          <Label>Upload Medical License (optional but recommended)</Label>
          <div onClick={() => document.getElementById('licenseUpload').click()} style={{
            border: `2px dashed ${licenseFile ? C.green : C.border}`, borderRadius: 10,
            padding: "1.4rem", textAlign: "center", cursor: "pointer",
            background: licenseFile ? C.greenBg : "#fafcfd",
            transition: "all 0.2s",
          }}>
            {licenseFile
              ? <div style={{ color: C.green, fontWeight: 600, fontSize: 14 }}>✅ {licenseFile.name}</div>
              : <>
    ...
    <div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "1.2rem", marginBottom: "0.5rem" }}>
        <div style={{
          ...
          background: photo ? "transparent" : `linear-gradient(135deg, ${C.sky}, ${C.ocean})`,
          display: "flex", alignItems: "center", justifyContent: "center",
          fontFamily: "Georgia, serif", fontWeight: 700, fontSize: 24, color: "white",
          border: `2px solid ${C.border}`,
        }}>
          {photo ? <img src={photo} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "MV"}
        </div>
        <div>
          <Label>Profile Photo</Label>
          <label style={{
            display: "inline-flex", alignItems: "center", gap: 6, cursor: "pointer",
            background: "white", border: `1.5px solid ${C.border}`, padding: "6px 14px",
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: "0.8rem" }}>
        {[
          ...
          <button key={label} onClick={() => set(!val)} style={{
            ...
            background: val ? C.ocean : "white", color: val ? "white" : C.text,
            border: `1.5px solid ${val ? C.ocean : C.border}`,
          }}>{label}</button>
        ))}
      </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {hours.map((row, i) => (
          <div key={row.day} style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            {row.open
              ? <>
                  <input type="time" value={row.from} onChange={e => updateHours(i, "from", e.target.value)}
                    style={{ padding: "4px 8px", border: `1.5px solid ${C.border}`, borderRadius: 7, fontFamily: "inherit", fontSize: 13, outline: "none" }} />
                  <span style={{ fontSize: 12, color: C.muted }}>to</span>
                  <input type="time" value={row.to} onChange={e => updateHours(i, "to", e.target.value)}
                    style={{ padding: "4px 8px", border: `1.5px solid ${C.border}`, borderRadius: 7, fontFamily: "inherit", fontSize: 13, outline: "none" }} />
                </>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
        {ALL_INSURANCES.map(ins => (
          <ToggleChip key={ins} label={ins} active={selectedInsurance.includes(ins)} onClick={() => toggleInsurance(ins)} />
        ))}
      </div>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
        {ALL_LANGUAGES.map(lang => (
          <ToggleChip key={lang} label={lang} active={selectedLangs.includes(lang)} onClick={() => toggleLang(lang)} />
        ))}
      </div>
    ...
    <div>
      ...
      <div style={{ background: `linear-gradient(135deg, ${C.deep}, ${C.ocean})`, borderRadius: 14, padding: "1.3rem", marginBottom: "1.2rem", display: "flex", gap: "1rem", alignItems: "center" }}>
        <div style={{ width: 60, height: 60, borderRadius: "50%", overflow: "hidden", background: "rgba(255,255,255,0.15)", border: "2px solid rgba(255,255,255,0.3)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "Georgia, serif", fontWeight: 700, fontSize: 20, color: "white", flexShrink: 0 }}>
          {data.photo ? <img src={data.photo} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : "MV"}
        </div>
        <div>
          ...
          <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginTop: 2 }}>📍 {doc?.address}</div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={onBack} style={{ background: "transparent", color: C.muted, border: `1.5px solid ${C.border}`, padding: "0.75rem 1.2rem", borderRadius: 10, fontFamily: "inherit", fontSize: 14, cursor: "pointer" }}>← Edit</button>
        <button onClick={onSubmit} style={{ flex: 1, background: C.dusk, color: "white", border: "none", padding: "0.8rem", borderRadius: 10, fontFamily: "inherit", fontSize: 15, fontWeight: 600, cursor: "pointer" }}>
          🚀 Submit Listing
        </button>
      </div>
    </div>
...
function Success() {
  return (
    <div style={{ textAlign: "center", padding: "2rem 1rem" }}>
      ...
      <div style={{ background: C.greenBg, border: `1px solid ${C.greenBorder}`, borderRadius: 12, padding: "1rem 1.2rem", marginBottom: "1.5rem", fontSize: 13, color: C.green, maxWidth: 380, margin: "0 auto 1.5rem" }}>
        📬 A confirmation email has been sent to your address. Check your inbox — and your spam folder just in case.
      </div>
      <button style={{ background: C.ocean, color: "white", border: "none", padding: "0.75rem 2rem", borderRadius: 10, fontFamily: "inherit", fontSize: 15, fontWeight: 600, cursor: "pointer" }}>
  ...
  return (
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", background: C.bg, minHeight: "100vh", color: C.text }}>
      ...
      {!submitted && step === 0 && (
        <div style={{ background: `linear-gradient(135deg, ${C.deep}, ${C.ocean})`, color: "white", padding: isMobile ? "1.8rem 1.2rem" : "2.2rem 2rem", textAlign: "center" }}>
          <div style={{ maxWidth: 560, margin: "0 auto" }}>
            <div style={{ fontSize: isMobile ? 20 : 24, fontFamily: "Georgia, serif", fontWeight: 700, marginBottom: 8 }}>
              Claim Your Free Listing on Your Doctor SD
            </div>
            <div style={{ fontSize: 14, opacity: 0.85, lineHeight: 1.7 }}>
              Help San Diego patients find you with accurate, up-to-date information. No fees. No paid rankings. Just your practice, listed honestly.
            </div>
      ...
      <div style={{ maxWidth: 620, margin: "0 auto", padding: isMobile ? "1.2rem 1rem 3rem" : "2rem 1.2rem 4rem" }}>
        {submitted ? <Success /> : (
          <Card>
            {!submitted && <StepIndicator current={step} isMobile={isMobile} />}
