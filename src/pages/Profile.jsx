...
function ReportForm({ npi, onClose, onSubmitted }) {
  ...
  return (
    <div>
      ...
      {error && <div style={{ fontSize: 12, color: "#c05050", marginBottom: 10 }}>{error}</div>}
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={handleSubmit} disabled={submitting} style={{ flex: 1, background: C.ocean, color: "white", border: "none", padding: "0.65rem", borderRadius: 8, fontFamily: "inherit", fontSize: 14, fontWeight: 600, cursor: submitting ? "default" : "pointer", opacity: submitting ? 0.7 : 1 }}>
...
function GoogleMap({ address, city }) {
  ...
  if (mapError) return (
    <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: `linear-gradient(135deg, rgba(26,107,138,0.08), rgba(77,184,212,0.12))`, borderRadius: 10, gap: 6, cursor: "pointer", color: C.muted, fontSize: 12 }}
      ...
      <span style={{ fontSize: 26 }}>🗺️</span><span>View on Google Maps</span>
    </div>
  );
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    async function fetchDoc() {
      try {
        const query = npi ? `/api/search?name=${npi}&limit=1` : `/api/search?name=${slug}&limit=1`;
  ...
  useEffect(() => {
    ...
    if (slug !== canonicalSlug) navigate(`/doctor/${canonicalSlug}`, { replace: true, state: { doc } });
  }, [doc, slug]);

  async function fetchReports(docNpi) {
    setReportsLoading(true);
    try {
      const res = await fetch(
        `${SUPABASE_URL}/rest/v1/community_reports?npi=eq.${docNpi}&order=created_at.desc&limit=20`,
  ...
  useEffect(() => { if (doc?.npi) fetchReports(doc.npi); }, [doc]);
  ...
  useEffect(() => {
    ...
    async function fetchNearby() {
      try {
        ...
        const data = await res.json();
        const others = (data.results || []).filter(p => p.npi !== doc.npi && p.address?.toLowerCase() === doc.address?.toLowerCase());
    ...
    <div style={{ fontFamily: "system-ui, -apple-system, sans-serif", background: C.bg, minHeight: "100vh" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", position: "relative", zIndex: 1, display: "flex", gap: isMobile ? "1rem" : "1.8rem", alignItems: "flex-start", flexWrap: isMobile ? "wrap" : "nowrap" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              ...
              {reports.length > 0 && <Badge icon="💬" text={`${reports.length} community report${reports.length !== 1 ? "s" : ""}`} />}
            </div>
          </div>
          {!isMobile && (
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", padding: isMobile ? "1rem" : "1.5rem 1.2rem 3rem", display: "flex", gap: "1.2rem", alignItems: "flex-start" }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          {isVerified ? (
            <>
              {doc.bio && <Section title={`About ${doc.name.split(",")[0]}`}><p style={{ fontSize: 14, lineHeight: 1.8, color: C.text, margin: 0 }}>{doc.bio}</p></Section>}
              {doc.insurance?.length > 0 && (
                <Section title="Insurance Accepted">
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{doc.insurance.map(ins => <span key={ins} style={{ fontSize: 13, padding: "5px 12px", borderRadius: 8, background: C.bg, color: C.text, border: `1px solid ${C.border}` }}>{ins}</span>)}</div>
              ...
              {doc.languages?.length > 0 && (
                <Section title="Languages Spoken">
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{doc.languages.map(l => <span key={l} style={{ fontSize: 13, padding: "5px 12px", borderRadius: 20, background: "rgba(26,107,138,0.07)", color: C.ocean, border: "1px solid rgba(26,107,138,0.15)" }}>{l}</span>)}</div>
                </Section>
              )}
            </>
          ) : (
            <Section title="Practice Information">
              <div style={{ marginBottom: "1rem" }}><ClaimNudge navigate={navigate} /></div>
          ...
          {nearbyProviders.length > 0 && (
            <Section title="Other Providers at This Address">
              <div style={{ fontSize: 13, color: C.muted, marginBottom: "0.8rem" }}>Other licensed providers at {doc.address}, {doc.city}</div>
              <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: "0.6rem" }}>
                {nearbyProviders.map(p => <NearbyProviderCard key={p.npi} doc={p} navigate={navigate} />)}
              </div>
            </Section>
          )}
          ...
          <Section title={`Community Updates${reports.length > 0 ? ` (${reports.length})` : ""}`}>
            ...
            {reportsLoading ? (
              <div style={{ fontSize: 13, color: C.muted, fontStyle: "italic", padding: "0.5rem 0 1rem" }}>Loading reports…</div>
            ) : reports.length > 0 ? (
              ...
              <div style={{ color: C.muted, fontSize: 13, fontStyle: "italic", padding: "0.5rem 0 1rem" }}>No community reports yet — be the first to share an update!</div>
            )}

            <div style={{ marginTop: "0.5rem" }}>
              {showReportForm
                ? <ReportForm npi={doc.npi} onClose={() => setShowReportForm(false)} onSubmitted={() => { setShowReportForm(false); fetchReports(doc.npi); }} />
        ...
        {!isMobile && (
          <aside style={{ width: 240, flexShrink: 0 }}>
            <div style={{ background: "white", border: `1.5px solid ${C.border}`, borderRadius: 14, padding: "1.2rem", marginBottom: "1rem", position: "sticky", top: 72 }}>
              ...
              {doc.address && <div style={{ display: "flex", gap: 8, marginBottom: 10, fontSize: 13, color: C.text, lineHeight: 1.5 }}><span style={{ flexShrink: 0 }}>📍</span><span>{doc.address}, {doc.city}, CA</span></div>}
              {doc.phone && doc.phone !== "Call for number" && <div style={{ display: "flex", gap: 8, marginBottom: 10, fontSize: 13, color: C.text }}><span>📞</span><span>{doc.phone}</span></div>}
              {doc.address && <div style={{ height: 150, marginTop: 8, borderRadius: 10, overflow: "hidden", border: `1px solid ${C.border}` }}><GoogleMap address={doc.address} city={doc.city} /></div>}
              <a href={`https://maps.google.com/?q=${encodeURIComponent(`${doc.address}, ${doc.city}, CA`)}`} target="_blank" rel="noreferrer" style={{ display: "block", textAlign: "center", fontSize: 12, color: C.ocean, marginTop: 8 }}>Open in Google Maps ↗</a>
              {!isVerified && <button onClick={() => navigate("/pricing")} style={{ width: "100%", marginTop: "1rem", background: C.ocean, color: "white", border: "none", padding: "0.65rem", borderRadius: 8, fontFamily: "inherit", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>🏥 Claim This Listing</button>}
            </div>
          </aside>
        )}
      </div>
      ...
      {isMobile && (
        <div style={{ padding: "0 1rem 2rem" }}>
          <Section title="Contact & Location">
            {doc.address && <div style={{ display: "flex", gap: 10, marginBottom: 10, fontSize: 14 }}><span>📍</span><span>{doc.address}, {doc.city}, CA</span></div>}
            {doc.phone && doc.phone !== "Call for number" && <div style={{ display: "flex", gap: 10, marginBottom: 10, fontSize: 14 }}><span>📞</span><span>{doc.phone}</span></div>}
            {doc.address && <div style={{ height: 160, borderRadius: 10, overflow: "hidden", border: `1px solid ${C.border}` }}><GoogleMap address={doc.address} city={doc.city} /></div>}
            <a href={`https://maps.google.com/?q=${encodeURIComponent(`${doc.address}, ${doc.city}, CA`)}`} target="_blank" rel="noreferrer" style={{ display: "block", textAlign: "center", fontSize: 12, color: C.ocean, marginTop: 8 }}>Open in Google Maps ↗</a>
            {!isVerified && <button onClick={() => navigate("/pricing")} style={{ width: "100%", marginTop: "1rem", background: C.ocean, color: "white", border: "none", padding: "0.65rem", borderRadius: 8, fontFamily: "inherit", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>🏥 Claim This Listing</button>}
          </Section>
        </div>
